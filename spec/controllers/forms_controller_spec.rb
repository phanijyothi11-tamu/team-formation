...
RSpec.describe 'TeamFormation' do
  let(:form) do
    double('Form',
      form_attributes: [
        double('Attribute', id: 1, name: 'gender', weightage: 1.0),
        double('Attribute', id: 2, name: 'technical', weightage: 1.0),
        double('Attribute', id: 3, name: 'communication', weightage: 1.0),
        double('Attribute', id: 4, name: 'leadership', weightage: 1.0)
      ]
    )
  end
  ...
  describe '#populate_teams_based_on_gender' do
    let(:result) { populate_teams_based_on_gender(students_data) }
    let(:section_a_teams) { result["Section A"][:teams] }

    it 'creates the correct number of teams' do
      expect(section_a_teams.length).to eq(3)
    end

    it 'handles odd number of females correctly' do
      last_team = section_a_teams.last
      female_count_in_last_team = last_team.count { |student|
        students_data["Section A"][:form_responses].find { |r| r.student == student }
          .responses[1] == 'female'
      }
      expect(female_count_in_last_team).to eq(3)
    end

    it 'distributes "other" gender students to teams with females' do
      section_a_teams.each do |team|
        if team.length >= 2
          other_students_in_team = team.count { |student|
            students_data["Section A"][:form_responses].find { |r| r.student == student }
              .responses[1] == 'other'
          }
          expect(other_students_in_team).to eq(1)
        end
      end
    end

    it 'maintains high-low score distribution in two-female teams' do
      two_female_teams = section_a_teams.select { |team|
        team.count { |student|
          students_data["Section A"][:form_responses].find { |r| r.student == student }
            .responses[1] == 'female'
        } == 2
      ...
      two_female_teams.each do |team|
        female_students_in_team = team.select { |student|
          students_data["Section A"][:form_responses].find { |r| r.student == student }
            .responses[1] == 'female'
        }

        female_scores = female_students_in_team.map { |student|
          response = students_data["Section A"][:form_responses].find { |r| r.student == student }
          (response.responses[2] + response.responses[3] + response.responses[4]) / 3.0
        }.sort.reverse

        expect(female_scores[0] - female_scores[1]).to be > 5 # Ensuring score difference
      end
    end

    it 'stores remaining male students correctly' do
      remaining_males = result["Section A"][:remaining_male_students]
      expect(remaining_males.length).to eq(3)
      expect(remaining_males).to contain_exactly('Mike', 'John', 'David')
    end

    it 'creates teams of correct sizes' do
      team_sizes = section_a_teams.map(&:length)
      expect(team_sizes[0..1]).to all(be <= 4)
      expect(team_sizes.last).to be <= 3
    end

    it 'places students in exactly one team' do
      all_placed_students = section_a_teams.flatten
      expect(all_placed_students.uniq.length).to eq(all_placed_students.length)
    end
  end

  describe '#calculate_weighted_average' do
    let(:student_response) do
      students_data["Section A"][:form_responses].first
    end

    it 'calculates weighted average correctly' do
      average = calculate_weighted_average(student_response)
      expected_average = (95 + 90 + 85) / 3.0 # Simple average for equal weights
      expect(average).to be_within(0.01).of(expected_average)
    end
  end

  describe '#find_gender_attribute' do
    it 'finds the gender attribute correctly' do
      gender_attr = find_gender_attribute(form)
      expect(gender_attr.name.downcase).to eq('gender')
    end
  end
end
